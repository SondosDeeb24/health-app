...
const sign_in = async (req: Request, res: Response): Promise< any > => { // this function will return Response object that have return value( mostly in my case here it's string )
    ...
    try {
        ...
        const [adding_user] = await database.query< ResultSetHeader>(
            "INSERT INTO health_app.users (username, user_fullname, user_blood_type, user_sex, user_address, user_phone, user_email, user_birth_date, user_hashed_password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [username, user_fullname, user_blood_type, user_sex, user_address, user_phone, user_email, user_birth_date, hashed_password]);
