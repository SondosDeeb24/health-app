...
const view_appointments = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!doctor_id) {
            [appointments_found] = yield database_connection_1.default.query(`SELECT * FROM health_app.appointments WHERE appointment_status = available AND doctor_id = ${doctor_id}`);
        }
        else {
            [appointments_found] = yield database_connection_1.default.query(`SELECT * FROM health_app.appointments WHERE appointment_status = available`);
        }
